...
import { createReview, deletReview, getAll, setTourAndUserIds, updateReview } from '../controllers/reviewController';
import { EUserRole } from '../models/userModel';

const router = express.Router({ mergeParams: true });

router.route('/').get(getAll).post(verfiyToken, restrictTo([EUserRole.USER]), setTourAndUserIds, createReview);
